...
class CompletedScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("Completed Tasks",style: TextStyle(color: Colors.white),), backgroundColor: const Color.fromARGB(255, 13, 78, 131)),
      body: Container(
        width: screenWidth,
        height: screenHeight,
       decoration: const BoxDecoration(
        ...
        child: Consumer<AuthViewModel> (builder: (context,authprovider,child) {
            return ListView.separated(
                    shrinkWrap: true,
                       itemCount: authprovider.completed.length,
                       separatorBuilder: (BuildContext context, int index) => SizedBox(height: 10),
                       itemBuilder: (BuildContext context, int index) {
                        Task myTask = authprovider.completed[index];
